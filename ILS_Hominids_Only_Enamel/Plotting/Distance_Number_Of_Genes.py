...
Variants_Output_Folder=open('Variant_Table_Hominids.tsv','w')
...
for nop in range(1,Max_Proteins+1):
    ...
    Variants_Output_Folder.write(F'{np.mean(Variants_Per_Protein_Number[nop])}\t{np.median(Variants_Per_Protein_Number[nop])}\t{min(Variants_Per_Protein_Number[nop])}\t{max(Variants_Per_Protein_Number[nop])}\n')
...
for PROTEIN_N in SORTED_N_PROTEINS: #### cycle through all protein numbers
    ...
    AVG_SUP_T1=np.mean(BOOTSTRAPS_SUPPORTING_T1)
...
for PROTEIN_N in SORTED_N_PROTEINS: #### cycle through all protein numbers
    ...
    AVG_SUP_T2=np.mean(BOOTSTRAPS_SUPPORTING_T2)
...
for PROTEIN_N in SORTED_N_PROTEINS: #### cycle through all protein numbers
    ...
    BOOTSTRAPS_SUPPORTING_T3 = [x for x in BOOTSTRAPS_SUPPORTING_T3 if isinstance(x, numbers.Number)] ### if somehow Nans are here
    AVG_SUP_T3=np.mean(BOOTSTRAPS_SUPPORTING_T3)
...
bp1 = ax.boxplot(BOOTSTRAPS[0], positions = np.arange(1, (Max_Proteins)*2, step=2)-0.5, sym='', widths=0.4,showfliers=False,showmeans=False, patch_artist=True)
bp2 = ax.boxplot(BOOTSTRAPS[1], positions = np.arange(1, (Max_Proteins)*2, step=2), sym='', widths=0.4,showfliers=False,showmeans=False, patch_artist=True)
bp3 = ax.boxplot(BOOTSTRAPS[2], positions = np.arange(1, (Max_Proteins)*2, step=2)+0.5, sym='', widths=0.4,showfliers=False,showmeans=False, patch_artist=True)
